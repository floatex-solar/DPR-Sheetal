...
const TypeSection = ({ index, types, section, onChange, onDelete }) => {
  const [open, setOpen] = useState(true);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

  const { typeName, machines } = section;
  ...
  const handleTypeChange = (newType) => {
    onChange({
      ...section,
      typeName: newType,
      machines: [], // reset machines when type changes
    });
  };
  ...
  const updateMachine = (machineIndex, updatedMachine) => {
    onChange({
      ...section,
      machines: machines.map((m, i) =>
        i === machineIndex ? updatedMachine : m
      ),
    });
  };
  ...
  const removeMachine = (machineIndex) => {
    onChange({
      ...section,
      machines: machines.filter((_, i) => i !== machineIndex),
    });
  };
                ...
                {machines.map((machine, mIdx) => (
                  <MachineSection
                    ...
                    onChange={(updatedMachine) =>
                      updateMachine(mIdx, updatedMachine)
                    }
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This type section including all machines and item entries will be
